...
def login(address):
    ...
    requests.get('https://robots.farm/api/account', params=params, headers=headers).json()
...
def is_available(address, private):
    ...
    if response['Next'] == '0001-01-02T00:00:00Z':
...
def get_reward(private):
    ts = int(time.time())
    message = f'Robots.farm play Quest {quest_id} {ts}'
    ...
    response = requests.get('https://robots.farm/api/play-quest', params=params, headers=headers)
    try:
...
def main(private):
    ...
    login(address)
    # is_available_list = is_available(address, private) # for multiaccounts
    is_available_list = (True, 0) # for quest 2 from maint account
